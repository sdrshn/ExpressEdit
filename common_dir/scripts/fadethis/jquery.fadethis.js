    ...
    Plugin.prototype = {
        ...
        _addElement: function ($elem, options) {
           // var metadata        = $elem.data( "plugin-options" ),
                eid             = $elem.data( "fadethis-id" ),
                localOptions    = $.extend({}, this.defaults, options),
                item = {
                    classId:    eid,
                    element:    $elem,
            ...
            //EE-< splice out old settings on resize if @media changes and browser resizes
             for (var i = _watch.length - 1; i >= 0; --i) {
                if (_watch [i].classId == eid) {
                    _watch.splice(i,1);
                    }
                }
            if (!localOptions.forceOff){ //EE  -> if force off chosen leave out as item
                _watch.push(item);
                }
            else  //EE
                item.element.stop(true).animate({'opacity':1,'duration':1000,'easing':'swing','left':0,'right':0,'bottom':0,'top':0,'visibility':'visible'})
    ...
    $.fn[ Plugin.globals.pluginName ] = function ( options ) {
        this.each(function() { 
            if ( !$.data( window, "plugin_" + Plugin.globals.pluginName ) ) {
                ...
                window.Plugin._addElement( $(this), options ) ; 
            }
        });
