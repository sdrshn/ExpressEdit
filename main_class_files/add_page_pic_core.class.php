...
function __construct(){
	...
	$instructions=" The maximimum Picture file size is limited to $maxup Mb ".$instructions.'';
	echo '
	<form  enctype="multipart/form-data" action="'.$postreturn.'" method="post" onsubmit="return edit_Proc.checkPicFiles(this,'.$maxbytes.',\'fileinput\','.$bbbwater.');">
	...
	if ($addvid){
		if (strpos(strtolower($vidname),'.flv')==false&&strpos(strtolower($vidname),'.swf')==false){
			$this->show_more('Add Play button to image');
			...
			printer::alertx('<p class="fsmnavy" title="After resizing to match widths your play button image height may differ from your start image for video  upload height so you may align the two according to their bottoms, centers, or tops">Choose Play Button Vertical Alignment with Image</p>');
			echo '
			...
			$this->show_more('Guideline for Creating a Custom Overlay/Watermark Image','noback','left editcolor','','full');		    
	...
	<tr><th>upload_max_filesize <br> php.ini</th><th>post_max_size <br> php.ini</th><th>Cfg::Pic_upload_max<br>Cfg_master.class.php or Cfg.class.php</th>
	</tr>
	<tr>
	...
	if ($bbb==='background_image_style')printer::printx('<p class="info smaller" title="Do Not Resize is particularly suited when repeating images (tiling) or for images intended to be larger or smaller than available background width"><input type="checkbox" name="no_image_resize" value="1">&nbsp;&nbsp;Do Not Resize Background Image</p>');
		...
		printer::printx('<p class="info smallest" title="Do Not Resize Animated Gifs, SVG images. "><input type="checkbox" name="no_image_resize" value="1">&nbsp;&nbsp;Do Not Resize Image<br>(Use for animated Gifs Svgs, &amp; Smaller filesize Images)</p>');
		}
	printer::close_print_wrap('resizing');	
	printer::pclear(10);
	$this->show_more('Add an Overlay/Watermark Image','','left info','Use a Watermark Image to be programmatically blended to your Uploaded Image. Watermarks are a means to prevent images from being downloaded and used by others, by merging your image with a transparent  watermark  image.');
	...
	printer::alertx('<p class="left info" title="After resizing to match widths your watermark image height may differ from your image to upload height so you may align the two according to their bottoms, centers, or tops">Choose Watermark Vertical Alignment with Image</p>');
	echo '
		<select class="editcolor editbackground" name="watermarkinfo[1]">       
		 <option value="center" selected="selected">Center to Center</option>
		 <option value="top">Top to Top</option>
	...
	printer::pclear(5); 
	echo '<p class="info left" title="After Uploading Your Custom Overlay/Watermark Image You Must Navigate back to Continue Your Normal Upload"> Upload Your Own Custom Watermark image
		...
		$this->show_more('Guideline for Creating a Custom Overlay/Watermark Image','','left maroon shadow');
	...
	printer::pclear(5); 
	echo '</div><!--wrapper-->';
	$this->show_close();//Watermarks option 
	printer::pclear(15);
	foreach (array('orig_id','image_noresize','www','clone_local_data','clone_local_style','expandfield','quality','append','wwwexpand','playbuttonadd','watermarkadd','addvid','contact','pgtbn','ttt','id_ref','id','fff','bbb') as $pass){
