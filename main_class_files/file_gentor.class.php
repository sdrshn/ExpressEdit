<?php
#ExpressEdit 2.0
class file_gentor extends file_generate{
...
function __construct($return=false){  
	 ...
	 if(!$return===false)return;
	   ...
	   if (isset($_REQUEST['photo_submit'])) {
          $load=new fullloader();
          $load->fullpath('includes/photo_batch_resize.class.php'); 
			   ...
			   if ($_REQUEST['choose']=='backup_folder') {
				 set_time_limit(300);
					...
					$Fdir=  (is_dir('F:/'))? 'F:/':'/media/VolF/';
						 ...
						 $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in '.$file;
						 self::$count_it=0;
						//self::backup_folder($home.$file,$dir.$fdrive.$file2);
						// $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in '.$file;
						//self::$count_it=0;
						 }
					 if (isset($_REQUEST['backup_downloads'])){
						 echo $msg11;
						 $dwnarr=array('Downloads/','Downloads1/','Downloads2/','Downloads3/','Downloads4/','Downloads5/','DownloadsWin7/','DownloadsAmazon/','DownloadsBrowser/','DownloadsFace/','DownloadsMaster/','DownloadsOrder/','DownloadsXNews/','DownloadsYoutube/');
						 foreach ($dwnarr as $dwndir){
							 self::backup_folder($Fdir.$dwndir,$dir.$dwndir);
							 $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in '.$dwndir;
							 }
						self::$count_it=0;
						} 
					 if  (isset($_REQUEST['backup_includes'])){
						 ...
						 $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in includes';
						 self::$count_it=0;
						 self::backup_folder($bashdir,$dir.$fdrive.'bash/');
						 $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in bash';
						 self::$count_it=0;
						 self::backup_folder($parent_inc_dir.$commondir,$dir.$fdrive.$commondir);
						  $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in common dir';
						  self::$count_it=0;
						 }
					 if (isset($_REQUEST['backup_htdocs'])){
						 echo $msg13;
						 self::backup_folder($htdocs,$dir.$fdrive.'htdocs/');
						 $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in htdocs';
						 self::$count_it=0;
						 }
					 if (isset($_REQUEST['backup_music'])){
						 echo $msg14;
						 self::backup_folder($hdrive.'My Music2/',$dir.'My Music2/');
						 $this->array_print[]= NL. 'total copied ' . self::$count_it. ' files in music';
						 self::$count_it=0;
						 }
					 
                         printer::vert_print($this->array_print);
					 }//end backup folder
			   }//END REQUEST CHOOSE
...
static function str_replace_data_mod($value) {
   ...
   #this first part is set up to remove dashes from filenames
	...
	if (strpos($value,$_SESSION[Cfg::Owner.'old'])===false){
		  if (strpos(strtolower($value),$_SESSION[Cfg::Owner.'old'])!==false){
		 printer::alert_neg('A lower case ');  printer::alert($value); printer::alert_neg(' has been found');
		 echo ' in '. $_SESSION[Cfg::Owner.'pagename'];
		#$value = str_replace($_SESSION[Cfg::Owner.'old'],$_SESSION[Cfg::Owner.'new'], $value);
		}
		return trim($value); 
	}
		 ...
		 echo NL.' in '. $_SESSION[Cfg::Owner.'pagename'];
...
function import_local(){
	 ...
	 foreach ($arr as $db){
		  ...
		  system($mysqlserver."mysql  -hlocalhost -uvwpkbpmy_sdrshn -p118cheznut  $db  <  $pathfile");
...
private function echo_data(){
	 ...
	 foreach ($table_array as $pagename){   
		  $q = "SELECT $field_data FROM ". $pagename . " where id=1";  
		  $r=$mysqlinst->query($q,__METHOD__,__LINE__,__FILE__,false); 
		  while ($rows=$mysqlinst->fetch_assoc($r,__LINE__,false)) {
			   for ($x=0; $x<count($field_array); $x++){
				    ...
				    echo  NL.$pagename .$field_array[$x].' = '. ${$field_array[$x]};
				    }
			   }//end while
		  }//end foreach
...
private function array_map_update($function){
	...
	$q="UPDATE $pagename $update where pic_order='$pic_order'";
	return $q;
	}//end function
...
function backup_folder($dir,$backupdir) {   
	...
	if ($directory_handle = opendir($dir)) {
		while (($file_handle = readdir($directory_handle)) !== false) { 
			if (empty(trim($file_handle)))continue;
               if (substr($file_handle,0,1)=='#'||substr($file_handle,0,2)=='.#'){
                    printer::alert_neg("File $dir.$file_handle has aberrant name");
                    continue;
                    }
			...
			if (is_dir($dir.$file_handle)) { //echo NL. NL. "directory is ".$dir.$file_handle.NL; 
				...
				file_generate::generate_folder($backupdir);
                    if (is_file($backupdir.$file_handle)){
                         if(!copy($dir.$file_handle,$backupdir.$file_handle)){
                              printer::alert_neg($file_handle ." failed to copy to $backupdir directory");
                              continue;
                              }
                         }
                    else  printer::alert_neg($file_handle ." misconfigured name to copy to $backupdir directory");
                            
				}
			else if (!is_file($dir.$file_handle)||strpos($backupdir,'serverbackup')!==false){//serverbackups need to be overwritten not renamed
			   	printer::alert_neg("Backup copy of this filename not copied: $dir$file_handle");
				continue;
				...
				continue;
                         ...
                         if  (is_dir($backupdir.$file_handle)){
                              printer::alert_neg("coping of is_file  $file_handle is already a directory on $backupdir$file_handle and the filename will be copied with an added extension of {$file_handle}.modbac");
                               copy($dir.$file_handle,$backupdir.$file_handle.'.modbac');
                               continue;
                               }
                                   
                    printer::alert("copy attempt on $backupdir$file_handle");
                     if(!copy($dir.$file_handle,$backupdir.$file_handle)){
                         printer::alert_neg("Backup copy of this missing filename not copied: $dir$file_handle");
                        continue;
                         }
                    }
			else if (strpos($backupdir,'serverbackup')!==false){//serverbackups need to be overwritten not renamed
			   	printer::alert_neu("Backup serverbackup copy of this filename not copied: $dir$file_handle");
...
static function run_db_table_automate(){#run localhost/run_db_table_automate.php
	...
	$do_function="update_style";
	 self::master_db_pagename_pass($do_function,true,'keywords');
	return;
	$do_function="add_blog_style";
	self::master_db_pagename_pass($do_function,true,'blog_style');
