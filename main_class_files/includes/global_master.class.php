...
#logged_in #login
	...
	//if ($this->login_forum){//default off
	//	new secure_login('forum',true); //Allows registration for users and username/password access
	//	}
	//if ($this->locked_pages&&$accesspage==false){//off by default and provides security for sites with privelidged business and pre patent info Cfg::Access_pages provides request parameter list of passwords to bypass login 
		//new secure_login('locked_domain',false);
		//}//
	#*************  End Restrict Access
	...
	$info='Sys::Self: '.Sys::Self.NL;
	$info.='Cfg::Expandgallery: '.Cfg::Expandgallery. NL;
	$info.='$this->gallery_global : '.$this->gallery_global . NL;
	
	if (GALLERY_INFO)echo "<div style=\"color:#00d00d; text-align: left; padding: 50px;\">$info</div>";
