...
$logged_in=session::session_check('logged_in');
 //must checkout whole editmode thing with render_html
(isset($_GET[Cfg::Admin_key]))&&$_SESSION[Cfg::Owner.'adminOn']=true; //this must be true to allow other other requests to be true allows special $_REQUESTS to show info info
 if(isset($_GET['advanced']))unset($_SESSION[Cfg::Owner.'advancedoff']);
$adminOn=(isset($_SESSION[Cfg::Owner.'adminOn'])&&$_SESSION[Cfg::Owner.'adminOn']===true)?true:false;
foreach($combine as  $val){
	$$val=0;
	if(($adminOn||$logged_in)&&in_array($val,$allon_arr)) $$val=session::session_batch_create($val,$allon_arr);//value becomes true only if correct  admin_key request is made  ?your admin key..  gives info only, not logging in for edit
...
//ftp_pass=(defined('Cfg::Ftp_pass'))?Cfg::Ftp_pass:'not defined';
//$pwall=strrev($ftp_pass);
//$dbname2=Cfg::Dbn_prefix.$webpage2.Cfg_loc::Domain_extension;
...
class Sys {
	...
	const AdminOn=ADMINON;//Boolean master control set by Admin_key allow other requests to provide debugging screen information and iframe backup visibility
...
#control sessions:
# adminOn: is master key to turn on other request keys controlled info and is turned on when Cfg::Debug_key is requested
...
# one can enter the value of the Cfg::Admin_key as a url request
#and the debugging request information will be rendered via turning
#Sys::AdminOn to true  
# furthermore With Sys::AdminOn and also being logged in  we turn on errors to screen reporting:    ini_set('display_startup_errors',1); and  ini_set('display_errors',1)  
# Sys::AdminOn enables iframe backup visibility...  see editing backup information for more on this
...
#Login   OR 
#enter the Cfg::Admin_key  as a request 
#ie  yoursite.com?youradminkeyxyz456    and  $adminOn & then Sys::AdminOn turned on    add  &showlist for list of request keys and what they do
...
#yoursite.com?alloff and all informatin keys will be turned off     Sys::AdminOn will remain true
