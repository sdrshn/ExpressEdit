...
static function check_gallery_master($method,$line,$file,$tablename,$return_list=false){ 
	...
	$q='select dir_ref from '.Cfg::Dbn_nav. ' where dir_is_gall=1';
...
static function key_up($array, $value, $return='val',$maxval=''){
    //print_r($array); echo "$value is value and return is $return and maxval is $maxval";
    $arr=$array;
...
static function validate_image_type($filename,$dir){ 
    ...
    //$allow_type=(!is_array(Cfg::Valid_pic_mime)) ? explode(',',$val_type):$val_type;		    
...
if ($maxi>4){
   ...
   if ($num==$maxi)   {  $array_send[]= $mini ; $array_send[]=$mini+1; } 
    
    
    else if ($num+1==$maxi)   {$array_send[]=$maxi; $array_send[]=$mini; } 
  else {$array_send[]=$num+1; $array_send[]=$num+2;} //   
 
   if ($num==$mini)   { $array_send[]=$maxi; $array_send[]=$maxi-1;}  
    elseif ($num-1==$mini) {$array_send[]=$mini; $array_send[]=$maxi;}
      else {$array_send[]=$num-1; $array_send[]=$num-2;} 
}
else {
