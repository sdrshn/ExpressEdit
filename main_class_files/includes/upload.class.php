...
printer::vert_print($_FILES);   
	$upload_dir=(!empty($upload_dir))?trim($upload_dir,'/').'/':'';
	...
	$uploadverification=false;
     if (isset($_FILES['upload'.$formcount])) {  
	   ...
	   //$allowed=explode(',',Cfg::Valid_pic_mime);
	   $msg='';
	   if ($_FILES['upload'.$formcount]['error'] > 0) {if  (Sys::Debug) Sys::debug(__LINE__,__METHOD__,__FILE__);
		  // Print a _ENV based upon the error.
		  switch ($_FILES['upload'.$formcount]['error']) {
			  case 1:
				  $msg= 'The file exceeds the upload_max_filesize setting in php.ini.';
				  break;
			  case 2:
	   ...
	   if (in_array($_FILES['upload'.$formcount]['type'], $allow_type) && $validated) {
			...
			if (move_uploaded_file($_FILES['upload'.$formcount]['tmp_name'], $newfile)) { //store the temp file to uploads directory
				...
				/*if ($_FILES['upload'.$formcount]['type']=='application/octet-stream'){ 
					$msg2='Someone has just uploaded application/octet stream and the system derived mime filetype is:  '.$last_line;
					mail::alert($msg2,'app/octet-stream upload');
					}*/
				//printer::alert($msg);
		...
		mail::alert('file_upload not found');
		return;
		}
