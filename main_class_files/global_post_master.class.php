...
function gallery($data,$gall_ref=''){   
	...
	if ($this->edit&&($this->clone_local_style||!$this->is_clone)){
		...
		if (!$this->master_gallery){ 
			...
			printer::printx('<p class="bold">1.<input type="radio" value="expand_preview_multiple" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked1.'>Use Multiple Preview Images Per Row for Separate Preview Display.</p>');
			printer::printx('<p class="bold">2.<input type="radio" value="expand_preview_single_row" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked2.'>Use a Single Preview Images Per Row for  Separate Preview Display </p>');
			printer::printx('<p class="bold">3.<input type="radio" value="expand_preview_single_pic" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked3.'>Use one Preview Image only to open into full gallery </p>');
			...
			printer::printx('<p class="bold">4.<input type="radio" value="preview_under_expand" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked4.'>Preview Images Under the Current Expanded Image</p>'); 
			printer::printx('<p class="bold">5.<input type="radio" value="expand_preview_none" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked5.'>No Preview Images</p>');
			...
			printer::printx('<p class="bold">6.<input type="radio" value="slide_caption" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked6.'>Auto slide Show Mode with mini captions</p>');  
			printer::printx('<p class="bold">7.<input type="radio" value="slide" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked7.'>Auto slide Show Mode No captions</p>');
			printer::pclear(5);
...
printer::pclear(5);
		...
		$checked='checked="checked"';
			...
			printer::printx('<p class="bold">8.<input type="radio" value="photoswipe_multiple" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked8.'>Display with Multiple Representative Images Per Row</p>');
			printer::printx('<p class="bold">9.<input type="radio" value="photoswipe_single_row" name="'.$data.'_blog_data2['.$preview_index.']" '.$checked9.'>Display with a Single Image Per Row</p>');
			printer::pclear(5);
			...
			printer::alert('Highslide is older  gallery utilizing iframes that make it difficult to add further customization options. It make work as is for you as is otherwise we recommend slippry or photoswipe');
			$checked='checked="checked"';
...
function nav_menu($data,$dir_menu_id,$text){//blog_data6 available
	...
	$menutype=($this->blog_tiny_data2!=='force_vert')?Cfg::Horiz_nav_class:$this->blog_tiny_data2;
	static $inc=0;   $inc++;
	...
	if ($count>0){
		...
		$this->navobj->nav_repeat_submenu=($nav_params[$nav_repeat_submenu_index]!=='repeat')?false:true;
		 $this->navobj->render_menu($dir_menu_id,$menutype.$datainc);
           echo <<<eol
	
     <script>
     \$( function(){
          \$( '.$this->dataCss .nav_gen li:has(ul)' ).doubleTapToGo();
          });
     </script>
eol;
          }
	...
	if(!$this->edit)return;
	printer::pclear();
	$this->main_menu_check[]=array($this->blog_data1,$menutype.$datainc,$this->data,$this->current_net_width); //for creating utility and expand menus
	 #navstyle
	printer::pclear(15);
...
#navwidth		 
	...
	$nav_link_height=(is_numeric($nav_params[$nav_link_height_index])&&$nav_params[$nav_link_height_index]>24&&$nav_params[$nav_link_height_index]<101)?'height:'.$nav_params[$nav_link_height_index].'px;':''; 
     ...
     if (!Sys::Quietmode&&($this->clone_local_style||!$this->is_clone)){#    PROCEEd FOr NORMAL
		 ############### End Nav Options ###############
		$this->show_more('Detailed Link Syling &amp; Custom Menu Opts', '','','',500,'','float:left',true);
		$this->print_wrap('detail link');	 
		//echo '<div class="ramana editbackground  editfont maxwidth500  editfont floatleft" style="width:auto;" id="navstylewidth'.$inc.'t">';
		 printer::pspace(10);
		if($nav_params[$nav_force_caps_index]!=='force'){
			printer::alertx('<p class="fsminfo highlight" title="Captialize All Main Menu and Sub Menu Link Titles/Names"><input type="radio" value="force"   name="'.$data.'_'.$nav_name_ref.'_arrayed['.$nav_force_caps_index.']">Use Uppercase Text Only</p>');
			}
		else {
			printer::alertx('<p class="fsminfo highlight" title="Captialize All Main Menu and Sub Menu Link Titles/Names"><input type="radio" value="0"   name="'.$data.'_'.$nav_name_ref.'_arrayed['.$nav_force_caps_index.']">Disable Use Of Uppercase Text Only</p>');
			}
		printer::pclear(5); 
		...
		##############   horiz or vert
		$checked1=($menutype!=='force_vert')?'checked="checked"':''; 
          $checked2=($menutype==='force_vert')?'checked="checked"':'';#full width option
		$pass_nav_class=($menutype==='force_vert')?'vert':'horiz';
		...
		printer::pspace(10);
		printer::printx('<p class="fsminfo editbackground  editfont floatleft '.$this->column_lev_color.' left">Customize Your Menu Links <br>Style the  background area around each link with borders, colors, images, box shadow, creating a link button if you wish, and set the distance between the &#34;link button&#34; areas: </p>');
     printer::pclear();  
     ...
     $this->edit_styles_close($data,'blog_data4','.'.$this->dataCss.' .nav_gen ul li a',$style_list,'Style General Menu Link LI A','','Styles You Make Here will affect Normal and  Hover and Active Links. Use Nav General Post styling to set the collective nav link alignment center or left. For additional Hover and Active Link effects Style Options Below these<br>To Enlarge Background Area for Images and Background Color Use Padding Spacing<br>To Enlarge Spacing between Background styled links use margin spacing');
     $style_list='background,padding_top,padding_bottom,padding_left,padding_right,margin_top,margin_bottom,margin_left,margin_right,borders,box_shadow,outlines,radius_corner,transform';
     printer::pclear(5);
     $this->edit_styles_close($data,'blog_data3','.'.$this->dataCss.' .nav_gen ul li',$style_list,'Additionally Tweak the  General Menu Link LI Element','',' Use Nav General Post styling to set nav link alignment center or left. ');
     ...
     printer::print_tip('Style Hover on Sub Menu Panel');
	...
	if (Sys::Quietmode||(!$this->clone_local_style&&$this->is_clone))return;//{#    PROCEEd FOr NORMAL
	     printer::pclear(5);
		$msg='Vertical Menu links look better with a uniform link  when background color or images are used whereas horizontal menus  make better use of space to auto adjust width matching the length of link text. Link Width management toggles between a block width of 100%  for  menus when available space is under 200px  and provides a  default maximum width of 200px for text lengthy horizontal links when greater than 200px';
          echo '<div  class="fsminfo editbackground  editfont left  floatleft" ><!--Link width Wrapper-->';
          if ($nav_params[$nav_link_width_management_index]==='nomanage'){
			$styleWidthShow='style="display:block;"';//shows in editor only
			$this->show_more('More info','','info tiny italic');
               printer::print_info('When in Responsive Grid Mode Check to enable a 200px maximum link width for link text. Very long text will wrap around instead '.$msg);
               $this->show_close('More info');
			printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'_arrayed['.$nav_link_width_management_index.']" value="1" >Enable Link Width Response Managment for RWD Grid mode');
			}
		else {
			$styleWidthShow='style="display:none;"'; //do not show option
               $this->show_more('More info','','info tiny italic');
               printer::print_info('Check to disable a Default Maximum Width Link for text greater than 200px. Optionally choose a specific link width '.$msg);
               $this->show_close('More info');
			printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'_arrayed['.$nav_link_width_management_index.']" onclick="edit_Proc.displaythis(\''.$data.'_navwidth_show\',this,\'#fdf0ee\')" value="nomanage" >Disable Link Width Response Managment');
			}
		echo '<div id="'.$data.'_navwidth_show" class="fsminfo editbackground  editfont left   floatleft" '.$styleWidthShow.'><!--Link width-->';
		...
		printer::alertx('<p class="editcolor editbackground  editfont editcolor left" >Optionally Set a Specific Link Width</p>'); 
		$this->mod_spacing($data.'_'.$nav_name_ref.'_arrayed['.$nav_link_width_index.']',$current_link_width,44,320,1,'px','none');
		echo '</div><!--Link width-->';
		...
		$current_link_width=(is_numeric($nav_params[$nav_sub_link_width_index])&&$nav_params[$nav_sub_link_width_index]>44&&$nav_params[$nav_sub_link_width_index]<321)?$nav_params[$nav_sub_link_width_index]:'none'; 
		printer::alertx('<p class="editcolor editbackground  editfont editcolor left" >Optionally Set Specific Sub Link Width</p>'); 
		$this->mod_spacing($data.'_'.$nav_name_ref.'_arrayed['.$nav_sub_link_width_index.']',$current_link_width,44,320,1,'px','none');
		...
		echo '<p class="highlight left"  title="Optionally set a Uniform Height for all the Menu Links. By Default Links have automatic Height Depending on The Number of Rows of Link TEXT. Set a specific Height here">Optionally Set a Fixed Link Height</p>';
		...
		$this->show_close('Detailed Link Area');//<!--End Show More Edit Nav-->'; 
		 $this->show_more('Style/Enable/Customize @ Menu Icon Width','','','',600);
          ...
          printer::print_tip('Here you can style both the LI A and the parent LI element which affects individual links.');
          printer::print_info('Recomended to use LI A then for special needs use LI');
          $this->edit_styles_close($data,'blog_tiny_data8','.'.$this->dataCss.' .nav_gen .ulTop.menuRespond2 li a:hover','background,font_family,font_weight,font_color,text_shadow,line_height,letter_spacing,italics_font,small_caps,text_underline,borders,box_shadow,outlines,radius_corner,transform','Style LI A: Hover Link For Both Main &amp; Sub Menu Link if different than the main hover settings ','','When You Hover Over a Link with the cursor it wil change according to Any Styles set Here' );
          ...
          printer::print_wrap1('horiz adjust');
           $final_icon_open_position_left=$this->spacing($data.'_'.$nav_name_ref,$nav_icon_open_left_index,'return','Choose left positioning value','Use Left Horiz Position of absolute positioned opened sub menu','','','','');
          $checked='checked="checked"';
          if($nav_params[$nav_icon_open_left_index]==='zero'){
               $value=0;
               $msg1='<span class="orange">Remove</span> Express Left:0';
               }
          else {
               $value='zero';
               $msg1='Express Left:0';
          ...
          if($nav_params[$nav_icon_open_right_index]==='zero'){
               $value2=0;
               $msg2='<span class="orange">Remove</span> Express Right:0';
               }
          else {
               $value2='zero';
               $msg2='Express Right:0';
               }
          printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_open_left_index.']" value="'.$value.'" >'.$msg1);
          $final_icon_open_position_right=$this->spacing($data.'_'.$nav_name_ref,$nav_icon_open_right_index,'return','Choose right positioning value','Use Right Horiz Position of absolute positioned opened sub menu','','','','');
          printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_open_right_index.']" value="'.$value2.'" >'.$msg2);
          if ($nav_params[$nav_icon_open_right_index]==='zero')
          ...
          $final_icon_open_position_top=$this->spacing($data.'_'.$nav_name_ref,$nav_icon_open_top_index,'return','Choose top positioning value','Use top Vert Position of absolute positioned opened sub menu','','','',''); 
          printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_open_top_index.']" value="'.$value.'" >'.$msg1); 
          ...
          printer::print_wrap1('horiz adjust');
           $final_icon_only_open_position_left=$this->spacing($data.'_'.$nav_name_ref,$nav_icon_only_open_left_index,'return','Choose left positioning value','Use Left Horiz Position of open icon','','','','');
          $checked='checked="checked"';
          if($nav_params[$nav_icon_only_open_left_index]==='zero'){
               $value=0;
               $msg1='<span class="orange">Remove</span> Express Left:0';
               }
          else {
               $value='zero';
               $msg1='<span class="orange">Remove</span> Left:0';
               }
          if($nav_params[$nav_icon_only_open_right_index]==='zero'){
               $value2=0;
               $msg2='Remove Express Right:0';
               }
          else {
               $value2='zero';
               $msg2='Express Right:0';
               }
          printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_only_open_left_index.']" value="'.$value.'" >'.$msg1);
          $final_icon_only_open_position_right=$this->spacing($data.'_'.$nav_name_ref,$nav_icon_only_open_right_index,'return','Choose right positioning value','Use Right Horiz Position for open icon','','','','');
          printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_only_open_right_index.']" value="'.$value2.'" >'.$msg2);
          if ($nav_params[$nav_icon_only_open_right_index]==='zero')
          ...
          printer::print_wrap1('vert adjust');
          $final_icon_only_open_position_top=$this->spacing($data.'_'.$nav_name_ref,$nav_icon_only_open_top_index,'return','Choose top positioning value','Use top Vert Position of absolute positioned opened sub menu','','','',''); 
          printer::alert('<input type="checkbox" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_only_open_top_index.']" value="'.$value.'" >'.$msg1); 
         ...
         printer::pclear();
          ...
          echo '<p class="highlight left" title="Respond Menus Vertical when enabled means horizontal menus will respond vertical only when the menu-icon is clicked below the width threshold chosen">Choose whether Horizontal Menus Will Appear Vertically when Icon is clicked</p>';
          $checked1=($icon_vertical_choice!=='as_is')?'checked="checked"':'';
          $checked2=($icon_vertical_choice==='as_is')?'checked="checked"':'';
          echo '<p><input type="radio" value="vertical" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_vertical_response_index.']." '.$checked1.'>Vertical Only (Best if Menu has Sublinks)</p>';
          echo '<p><input type="radio" value="as_is" name="'.$data.'_'.$nav_name_ref.'['.$nav_icon_vertical_response_index.']." '.$checked2.'>As Is</p>';
          echo '</div><!-- Respond Vertical-->';
          printer::pclear(5);
          $this->submit_button(); 
          printer::close_print_wrap('rwd menu icon');
          $this->show_close('Style/Enable/Customize @ Menu Icon Width'); 
          echo'<p class="button'.$this->column_lev_color.' '.$this->column_lev_color.' editbackground  editfont editcolor floatleft underline shadowoff"> <a class="linkcolorinherit" href="navigation_edit_page.php?table_ref='.$this->pagename.'&amp;passnavclass='.$pass_nav_class.'&amp;data='.$data.'&amp;style='.$menutype.$datainc.'&amp;menuid='.$dir_menu_id.'&amp;postreturn='.Sys::Self.'&amp;pgtbn='.$this->pagename.'&amp;css='.$this->roots.Cfg::Style_dir.$this->pagename.'">Add Remove &amp; Edit Links for this Menu</a></p>';
		printer::pclear();
	...
	#blog_tiny_data3 display or horiz handled in the # maindiv specific for nav
	(!$this->is_clone||$this->clone_local_style)&&$this->navcss.='
	@media screen and (min-width: '.($bp_arr[0]+1).'px){';
	if ($nav_width_manage){
		if ($this->current_net_width>200)
		$this->navcss.='
